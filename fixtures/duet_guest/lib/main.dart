// Headless duet guest driver: NO runApp, NO Ticker, NO widget tree.
//
// This is a driver, not a library — it is what
...
// observes — how many pushes arrived, what the host replied to hostile input,
// the exact doubles it wrote — is published by writing one map at
...
Future<void> main() async {
  ...
  duet.onPush = (DuetNotification n) {
    ...
    _log('PUSH #${report.pushes} path=${n.path} value=${n.value}');
  ...
  if (!await _awaitHost(duet)) {
    _log('ERROR the host never answered on ${kDuetChannel.name}; giving up');
    return;
  }
  ...
  await _publish(duet, report);
  _log('FINAL pushes=${report.pushes}');
}

/// Writes the whole report at [kReportPath].
///
/// Never throws: this runs from a push handler as well as from `main`, and a
/// failed report is something the host observes as a missing update — there is
/// nothing useful this guest could do about it, and taking the isolate down
/// would destroy the very evidence the host is waiting for.
Future<void> _publish(DuetClient duet, _Report report) async {
  try {
    await duet.set(kReportPath, report.toValue());
  } on Object catch (e) {
    _log('ERROR publishing the report failed: $e');
  }
}
...
Future<bool> _awaitHost(DuetClient duet) async {
  final Stopwatch elapsed = Stopwatch()..start();
  while (elapsed.elapsed < kHandshakeTimeout) {
    try {
      await duet.get('counter');
      _log('host answered after ${elapsed.elapsedMilliseconds}ms');
      return true;
