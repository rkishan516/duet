...
//!
//! ```text
//! cargo run -p duet-backend-windows --example webview_commands
//! ```
//!
//! An **example**, not a test: this needs a real window and a pumped event
//! loop. Windows `tao` can build a loop on a worker thread
//! (`with_any_thread`; `src/sink.rs`'s tests use it for the proxy path), but
//! `cargo test`'s harness provides neither a window nor a pumped loop, and a
//! WebView2 webview needs both.
//!
...
//! it genuinely appears on a physical display. What an autonomous run cannot
//! supply is a human at the keyboard: nothing here touches mouse or keyboard
//! input, and `spikes/spike-b-windows` found that even posted synthetic input
//! reaches Flutter but not WebView2 (FINDINGS.md, W-F7).
//!
...
/// `wry`'s IPC handler on the platform thread, never the runtime's core thread.
#[command]
...
/// script's return value through WebView2's `ExecuteScript` JSON
/// serialization, so a returned JSON string comes back double-encoded. macOS
/// Spike B lost real time to exactly that, and `spikes/spike-b-windows`
/// (W-F8) confirmed the same behavior on WebView2.
///
...
/// the handler before `build()` yields the `WebView`.
