...
//! needed when running from another directory. That bundle's `flutter_assets`
//! carries **both** Dart drivers; this example selects the two-guest one by
//! seeding [`MODE_PATH`] with [`DUET_MODE`] (see `fixtures/duet_guest/lib/guest_support.dart`).
//!
...
//! no reachable on-screen WindowServer), this machine has a real display
//! session: the `tao` window genuinely appears on screen and `wry` renders
//! into it. What stays unverified is real mouse and keyboard input — these
//! runs are autonomous, with nobody at the keyboard — and synthetic input is
//! no substitute: `spikes/spike-b-windows/FINDINGS.md` W-F7 found posted
//! messages reach Flutter's gesture pipeline but never register in WebView2,
//! so input is out of scope here entirely. The Flutter engine runs
//! **headless** — booted with no view controller (headless is simply how the
//! C API composes, W-F3) and never given one — because none of these claims
//! need a view, and an engine that never creates its controller never touches
//! the controller-owns-the-engine hazard W-F1 documents; teardown goes through
//! `destroy_renderer`, which owns that distinction either way.
//!
//! Only a debug/JIT Windows bundle (`flutter_assets/` with `kernel_blob.bin`)
//! has ever been built in this environment, so nothing here says anything
//! about release/AOT Dart.
//!
//! # One event loop, two renderers
//!
//! `wry` needs a `tao` event loop; `flutter_state.rs` has none and pumps the
//! thread's Win32 message queue directly. They cannot both be true in one
//! process, so this file uses the `tao` loop as the single driver and boots
//! the Flutter engine alongside it — the arrangement Spike B measured
//! (`spikes/spike-b-windows`: a `tao` event loop, a Flutter engine and a `wry`
//! webview coexisting on one main thread, 723/723 proxy events over 180 s).
...
use duet_backend_windows::{DuetEvent, FlutterSurface, ProxySink, WebviewSurface, WinBackend};
...
/// script's return value through WebView2's `ExecuteScript` JSON serialization,
/// so a returned `JSON.stringify(x)` comes back double-encoded (macOS Spike B
/// lost real time to exactly this, and `spikes/spike-b-windows/FINDINGS.md`
/// W-F8 confirmed the same behavior on WebView2). Tolerates running before the
...
struct App {
    ...
    /// the engine shuts down — the engine holds the handler's `user_data`
    /// pointer past registration (see [`FlutterSurface`]'s `Drop`).
...
fn main() {
    ...
    // (headless is simply how the C API composes — W-F3) and nothing here ever
    // calls `attach_view`, so the Flutter side gets no window and no HWND. See
...
/// `wry` installs the handler before `build()` yields the `WebView`), and
...
///
/// The surface first: it owns the `wry` `WebView`, whose WebView2 child HWND
/// lives inside the window it was built in, and dropping the window out from
/// under it would be the reverse of how it was built.
...
fn report_and_exit(app: &mut App, probe: &Probe) -> ! {
    ...
    // engine holds the handler's `user_data` pointer past registration — see
    // `FlutterSurface`'s `Drop`. `destroy_renderer` is then what actually
    // reclaims the engine's memory (W-F1).
