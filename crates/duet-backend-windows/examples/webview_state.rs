...
//!
//! Run: `cargo run -p duet-backend-windows --example webview_state`
//!
...
//! assertion; Windows relaxes that via `with_any_thread`, which is why
//! `src/sink.rs`'s closed-loop test runs un-`#[ignore]`d here — but a live
//! WebView2 webview in a real window is still nothing `cargo test` provides.)
//!
//! # What this proves, and what it cannot prove here
//!
//! This machine has a real display session — the window genuinely appears on
//! a physical screen, unlike every macOS measurement, which ran without a
//! reachable on-screen WindowServer. What the example *asserts* is still only
//! what is observable from Rust: that JavaScript executes, that
//! `with_ipc_handler` delivers guest messages to Rust, that `evaluate_script`
//! delivers host scripts to the guest, and that the store is genuinely shared
//! between the two. All of that is in-process.
//!
...
//! `spikes/spike-b-windows` found posted synthetic input reaches a Flutter
//! view but *not* WebView2 (FINDINGS.md, W-F7); nothing in this example
//! touches input, so it neither confirms nor contradicts that.
//!
...
//! through WebView2's `ExecuteScript`, which JSON-serializes the completion
//! value on the native side, so returning `JSON.stringify(x)` comes back
//! double-encoded — quoted and escaped. macOS Spike B lost real time to
//! exactly this, and `spikes/spike-b-windows` confirmed the rule carries to
//! WebView2 verbatim (FINDINGS.md, W-F8). [`PROBE_JS`] therefore returns an
...
use duet_backend_windows::{DuetEvent, ProxySink, WebviewSurface};
