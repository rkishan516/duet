//! The Windows webview surface: a `wry` WebView (WebView2) wired to the
//! shared store.
//!
...
//!
//! This module is nearly identical to its macOS sibling — `wry` is the
//! cross-platform seam doing that work. What differs underneath: IPC arrives
//! via WebView2's `WebMessageReceived` (a COM event callback) instead of a
//! `WKScriptMessageHandler`, and `spikes/spike-b-windows` verified both
//! directions live — `window.ipc.postMessage` traffic and 723/723
//! `evaluate_script_with_callback` round trips (FINDINGS.md, criteria 2–4).
...
impl WebviewSurface {
    ...
    /// on Windows that includes a missing WebView2 Runtime.
    pub fn new(
        window: &Window,
        store: StoreHandle,
        subscriber: SubscriberId,
    ...
    pub fn with_commands(
        ...
        let webview = WebViewBuilder::new()
            ...
            .with_ipc_handler(move |request| {
                ...
                // states for its C trampoline. `wry`'s Windows IPC handler is
                // invoked from WebView2's `WebMessageReceived` COM callback,
                // so a panic unwinding out of here crosses into COM frames —
                // undefined behavior, in practice an abort. `handle_text` is
                ...
                // evaluated script: wry JSON-serializes a script's completion
                // value on the native side (WebView2's `ExecuteScript` does
                // this too — spike finding W-F8), which would double-encode
                // the JSON. Spike B on macOS hit exactly that bug.
                //
    ...
    /// `wry` JSON-serializes a script's completion value on the native side
    /// (WebView2's `ExecuteScript` returns results as JSON), so a script that
    /// returns an already-stringified JSON string comes back double-encoded —
    /// macOS Spike B hit exactly that, and `spikes/spike-b-windows` (W-F8)
    /// confirmed the same behavior on WebView2. Always return a plain
...
/// without a window. Everything else on this path needs a live `WebView`,
/// which needs the WebView2 Runtime and a real window — which is why the
/// platform backends are excluded from ubuntu CI entirely; a size cap that
/// only a human on the right OS can exercise is a size cap nobody checks.
///
...
mod tests {
    ...
    // the WebView2 Runtime and a real window the default `cargo test` harness
    // does not provide. What *is* tested is everything deliberately factored
    // out of the IPC closure so it is reachable without one — the size-cap
    // boundary, the routing around it, and the two const replies. The rest is
    // proved by the examples, driven against a real webview.
    ...
    fn a_panicking_handler_body_answers_the_guest_instead_of_unwinding() {
        ...
        // reply. Without the guard the panic unwinds into COM frames and takes
        // the process down, and the guest's call never settles.
