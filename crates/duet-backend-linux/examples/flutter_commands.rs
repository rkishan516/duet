...
//! (`libflutter_linux_gtk.so`) running real Dart invokes real `#[command]`
...
//! JavaScript, and a Dart guest speaks an `FlBinaryMessenger` channel and is
//! answered through the message's single-use response handle. A proof for one
//! says nothing about the other.
//!
//! # Build and run
//!
//! The Dart guest is a tracked fixture and must be compiled to a Linux `data`
//! directory first — a debug/JIT build (`flutter_assets/` carrying
...
//!   cargo run -p duet-backend-linux --example flutter_commands
//! ```
//!
//! `FLUTTER_LINUX_RENDERER=software` is required under WSLg, where there is no
//! usable GL ("Could not determine GL version"); GL-capable desktops may omit
//! it. `GDK_BACKEND=x11` is load-bearing for the headless boot below: on X11 a
//! hidden toplevel can be realized with ordinary GDK resources, while Wayland
//! surfaces only exist once mapped.
//!
...
//!
//! Nothing visual. The engine runs **headless** — booted with no window open,
//! so `start_renderer` realizes the implicit view into a private hidden
//! holding window instead; boot IS that realize, with Dart serving by the time
//! it returns (`spikes/spike-b-linux/FINDINGS.md` L-F1/L-F2), at the one price
//! Linux attaches to the headless path: the realized view can never move, so
//! this engine can never be attached afterwards (L-F3) — which none of these
//! claims would ever ask for. Only a debug/JIT bundle (`kernel_blob.bin`) has
//! ever been built in this environment, so nothing here says anything about
//! release/AOT Dart.
...
/// Seconds of main-context pump per turn. This is what gives the Dart isolate
/// its thread: without it the guest never runs and nothing here can happen.
...
/// --debug` (`flutter_assets/` + `icudtl.dat`, inside
/// `build/linux/x64/debug/bundle`).
///
/// Canonicalized to an absolute path before use, mirroring the other two
/// backends' examples: a relative path is resolved against whatever the
/// working directory happens to be, and an absolute one is the only shape
/// that cannot drift. `FlutterEngine::boot` canonicalizes again on its own;
/// doing it here as well means the line printed below names the directory the
...
fn main() {
    ...
    // GTK before the backend: this example never creates a tao event loop
    // (nothing here opens a window), so nobody else initializes GTK, and the
    // headless boot below builds its private holding window out of GTK
    // widgets on this thread's main context.
    if let Err(e) = gtk::init() {
        fail_setup(runtime, &format!("GTK could not initialize: {e}"));
    }

    let surface_id = SurfaceId::from_raw(1);
    let mut backend = LinuxBackend::new(&bundle);
    // No window is open, deliberately: on Linux that makes this boot headless
    // — the implicit view realizes into a private holding window (L-F1), and
    // because the realized view can never move (L-F3), this engine can never
    // be attached afterwards. Every claim here is channel traffic, so that
    // permanent headlessness costs nothing; a driver that ever wants a
    // visible surface must `open_window` before `start_renderer` instead.
...
/// Pumps the GTK main context for `secs`, which is what gives the Dart
/// isolate its thread back: the embedder's platform task runner schedules its
/// tasks as sources on this context — the same context the spike drove for
/// 180 s at 722/722 proxy events (`spikes/spike-b-linux/FINDINGS.md`) — and
/// iterating it is what runs them.
///
/// The same idiom as `examples/flutter_state.rs`, on purpose:
/// `iteration(true)` blocks until *some* source dispatches, and the one-shot
/// timeout is what guarantees one eventually does, bounding the turn without
