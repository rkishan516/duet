...
//!
//! Run: `FLUTTER_LINUX_RENDERER=software GDK_BACKEND=x11 cargo run -p duet-backend-linux --example webview_state`
//!
//! `FLUTTER_LINUX_RENDERER=software` is required under WSLg for the examples
//! that boot a Flutter engine (no usable GL there — see the crate root docs);
//! GL-capable desktops may omit it. This example boots no engine, so here the
//! variable is inert — kept only so every example in this crate shares one
//! run line. `GDK_BACKEND=x11` keeps GTK on the X11 path this port was
//! verified on.
//!
//! An **example**, not a test: `tao::EventLoop::run` never returns, and the
//! report can only end the process with `process::exit` — nothing a test
//! harness survives. (The macOS sibling also cites tao's main-thread
//! assertion; Linux relaxes that via `with_any_thread`, which is why
//! `src/sink.rs`'s closed-loop test runs un-`#[ignore]`d here — but a live
//! WebKitGTK webview in a real GTK window is still nothing `cargo test`
//! provides.)
//!
//! # What this proves, and what it cannot prove here
//!
//! This port was brought up under WSLg (see the crate root docs) — a real
//! display session, if a virtualized one: the window genuinely maps, and
//! WSLg presents it on the host desktop, unlike every macOS measurement,
//! which ran without a reachable on-screen WindowServer. What the example
//! *asserts* is still only what is observable from Rust: that JavaScript
//! executes, that `with_ipc_handler` delivers guest messages to Rust, that
//! `evaluate_script` delivers host scripts to the guest, and that the store
//! is genuinely shared between the two. All of that is in-process.
//!
//! **Not** provable here, and not claimed: that real mouse or keyboard input
//! reaches the webview — runs are autonomous, with nobody at the keyboard,
//! and nothing in this example touches input, so it neither confirms nor
//! contradicts anything about input delivery. (The Windows sibling cites its
//! spike's synthetic-input finding at this point; the Linux spike's findings
//! summarized in the crate root make no input claim this example could lean
//! on.)
//!
...
//! `wry`'s `evaluate_script_with_callback` JSON-serializes the script's
//! completion value on the native side, so returning `JSON.stringify(x)`
//! comes back double-encoded — quoted and escaped. macOS Spike B lost real
//! time to exactly this, and the rule holds verbatim on WebKitGTK (see
//! [`WebviewSurface::eval_with_callback`]). [`PROBE_JS`] therefore returns an
//! object literal. Note that the protocol itself never travels this way:
//! replies and pushes are *pushed in* via [`WebviewSurface::eval`] with the
//! payload embedded in the script, which has no such re-serialization step.
//!
//! # Staged, not raced
//!
...
fn main() {
    ...
    // Identical calling shape to the other two platforms; underneath, the
    // surface builds the wry webview into this tao window's default GTK vbox
    // (spike finding L-F7 — built against the window itself it would run
    // invisibly, because the GtkApplicationWindow already holds its one
    // permitted child).
...
/// before `build_gtk` yields the `WebView`), and [`DuetEvent::Notifications`]
/// is the push path.
