...
//! inside a real `FlEngine` writes into the store over a real platform
...
//! `fl_binary_messenger_set_message_handler_on_channel` ever hands a Dart
//! `BasicMessageChannel.send` to that function, or whether an
//! `fl_binary_messenger_send_on_channel` reaches the guest's own handler.
//! This example closes both halves of that loop over the real transport.
//!
//! # Build and run
//!
//! The guest is a tracked fixture and must be compiled to a Linux Flutter
...
//!   cargo run -p duet-backend-linux --example flutter_state
//! ```
//!
...
//! GL version"), and the crate already boots every engine with Impeller
//! disabled for exactly that configuration (L-F4) — while GL-capable desktops
//! may omit it. `GDK_BACKEND=x11` pins GDK to the X11 backend this port was
//! verified under.
//!
//! # What this proves, and what it cannot prove here
//!
//! Provable here: that a Dart isolate runs, that
//! `fl_binary_messenger_set_message_handler_on_channel` delivers guest
//! messages to Rust, that `fl_binary_messenger_send_on_channel` delivers host
//! pushes to the guest, and that the store is genuinely shared between the
//! two. All of that is in-process and observable from Rust.
//!
//! **Not** provable here, and not claimed: that real human input works. WSLg
//! projects X11 windows onto the Windows desktop, so windows genuinely appear
//! when an example opens one. This example opens none: it is deliberately
//! headless — no `tao` window, and no `attach_view` — because none of the
//! claims need one. Headless on Linux is not controller-less as it is on
//! Windows (W-F3): boot IS view realization (L-F1), so `start_renderer` with
//! no window open realizes the implicit view into a private hidden holding
//! window and the engine boots there, Dart serving by the time realize
//! returns (L-F2). Serving the store works identically; the price is that a
//! headless-booted engine can never be attached afterwards — the realized
//! view can never move (L-F3) — and this example never attempts it. The
//! holding window is never mapped, and an unmapped window draws nothing (the
//! spike measured frames all but stopping while hidden), so the macOS F1
//! backing-store concern has no purchase here. What stays unverified on this
//! machine is real mouse and keyboard input: every run is autonomous, with
...
//! [`duet_backend_linux::FlutterSurface`]'s own inbound size cap and its
...
//! channel handler is invoked on the GTK main context and replies inline too
//! (the spike drove 10,734 guest messages through exactly this
//! registration/reply pair), and the engine's task runner is itself a set of
//! `glib` sources on that same context — so all this driver needs is to keep
//! the default main context iterating. `glib::MainContext::iteration` plus a
//! one-shot timeout source does that directly, which keeps the turn structure
//! of this file explicit instead of hidden inside `tao`'s `ControlFlow`.
...
use duet_backend_linux::{FlutterSurface, LinuxBackend};
...
///
/// Generous: the work here is an engine boot (the realize `start_renderer`
/// performs returns with the isolate already serving — L-F2), a handshake, a
/// dozen channel round trips — two of which carry a megabyte — and the
/// guest's own observation window. All of it lands in well under 20 seconds
/// when it lands at all.
...
/// (`flutter_assets/` + `icudtl.dat`, inside the bundle). Overridable via env
/// var for other machines, matching `examples/lifecycle.rs`.
...
fn main() {
    ...
    // GTK must be initialized before any window exists — even the private
    // holding window a headless boot realizes the view into. `tao` does this
    // inside its event loop constructor; this example deliberately has no
    // event loop (see the module docs), so it initializes GTK directly.
    if let Err(e) = gtk::init() {
        fail_setup(runtime, &format!("could not initialize GTK: {e}"));
    }

    let surface_id = SurfaceId::from_raw(1);
    let mut backend = LinuxBackend::new(&data_dir);
    // Headless: with no window open for this surface, `start_renderer`
    // realizes the implicit view into a private hidden holding window — boot
    // IS realization on Linux (L-F1), so even a headless engine needs a
    // toplevel to be born in. Serving the store works identically; the price
    // is that this engine can never be attached afterwards (the realized view
    // can never move — L-F3), and nothing here ever calls `attach_view`. See
    // the module docs on why that is deliberate.
    ...
    // messenger holds the callback's `user_data` pointer past registration —
    // see `FlutterSurface`'s `Drop` — and `destroy_renderer` *forces* the
    // engine's dispose (see the crate's engine docs), after which there is
    // no live channel table to unregister from. The surface's own references
    // keep its send path memory-safe, nothing more. The private holding
    // window of this headless boot is destroyed in `destroy_renderer` too.
    drop(surface);
...
/// Runs the GTK main context for `secs`.
///
/// This is what gives the Flutter engine its thread back: the GTK embedder's
/// task runner is a set of `glib` sources on the default main context, so the
/// Dart isolate's microtasks, its timers, and every inbound platform message
/// are all serviced by iterating that context. Without it the guest never
/// runs and nothing in this file can happen.
///
/// Like the macOS sibling — and unlike the Windows one, whose only `unsafe`
/// blocks are its raw Win32 message pump — no `unsafe` is needed for any of
/// it: `glib` exposes both the context iteration and the one-shot timeout as
/// safe bindings, so this example contains no `unsafe` blocks at all. It
/// reaches the GTK world only through [`LinuxBackend`] and
/// [`FlutterSurface`], which own that unsafety.
fn pump_run_loop(secs: f64) {
    // `iteration(true)` blocks until *some* source dispatches; the one-shot
    // timeout is what guarantees one eventually does, bounding the turn.
    let expired = Rc::new(Cell::new(false));
    let flag = Rc::clone(&expired);
    glib::timeout_add_local_once(Duration::from_secs_f64(secs), move || flag.set(true));
