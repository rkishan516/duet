...
//! # Build and run
//!
//! The Dart guest is a tracked fixture and must be compiled to an
//! `App.framework` first — a debug/JIT build, the only kind this project has
...
//! only needed when running from another directory. That framework carries
//! **both** Dart drivers; this example selects the two-guest one by seeding
//! [`MODE_PATH`] with [`DUET_MODE`] (see `fixtures/duet_guest/lib/guest_support.dart`).
//!
...
//!
//! Nothing visual. This machine has no reachable on-screen WindowServer for
//! spawned processes (see the crate root docs): a `tao` window is created and
//! `wry` renders into it, but no human sees it and this example makes no claim
//! that anyone could. Real mouse and keyboard input are equally unproven here
//! and out of scope. The Flutter engine runs **headless** — booted with
//! `allowHeadlessExecution` and never given a view — because none of these
//! claims need one and a detached-view engine is where `FINDINGS.md` F1's
//! backing-store retry storm lives.
//!
//! Only a debug/JIT `App.framework` has ever been built in this environment,
//! so nothing here says anything about release/AOT Dart.
//!
//! # One event loop, two renderers
//!
//! `wry` needs a `tao` event loop; `flutter_state.rs` has none and pumps
//! `NSRunLoop` directly. They cannot both be true in one process, so this file
//! uses the `tao` loop as the single driver and boots the Flutter engine
//! alongside it — the arrangement Spike B measured
//! (`spikes/spike-b-macos`: a `tao` event loop, a Flutter engine and a `wry`
//! webview coexisting on one main thread, 709/709 proxy events over 180 s).
...
/// script's return value through `NSJSONSerialization`, so a returned
/// `JSON.stringify(x)` comes back double-encoded (Spike B lost real time to
/// exactly this). Tolerates running before the bootstrap script has executed,
/// which is normal on the first turns.
...
struct App {
    ...
    /// the engine shuts down — `shutDownEngine` does not clear the engine's
    /// handler map (see [`FlutterSurface`]'s `Drop`).
...
/// Overridable via env var for other machines, matching the sibling examples.
...
fn main() {
    ...
    println!("[two_guests] App.framework path: {framework}");
    ...
    // and nothing here ever calls `attach_view`, so the Flutter side gets no
    // window and no NSView. See the module docs on why that is deliberate.
...
///
/// The surface first: it owns the `wry` `WebView`, which is a subview of the
/// window's content view, and dropping the window out from under it would be
