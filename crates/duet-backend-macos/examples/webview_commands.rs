...
//!
//! ```text
//! cargo run -p duet-backend-macos --example webview_commands
//! ```
//!
//! An **example**, not a test: `tao`'s event loop must be built on the
//! process's main thread, which `cargo test`'s harness does not provide.
//!
...
//! `wry` renders into it, but no human sees it. Nothing here touches mouse or
...
/// script's return value through `NSJSONSerialization`, so a returned JSON
/// string comes back double-encoded. Spike B lost real time to exactly that.
///
