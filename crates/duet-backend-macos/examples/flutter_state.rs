...
//! directly, which proves the *routing* but says nothing about whether a
//! `FlutterBinaryMessenger` ever hands a Dart `BasicMessageChannel.send` to
//! that function, or whether a `sendOnChannel:message:` reaches the guest's
//! own handler. This example closes both halves of that loop over the real
//! transport.
//!
//! # Build and run
//!
//! The guest is a tracked fixture and must be compiled to an `App.framework`
//! first — a debug/JIT build, which is the only kind this project has ever
//! produced (release/AOT behaviour here is unverified):
//!
//! ```text
...
//! ```
//!
//! `DUET_APP_FRAMEWORK_PATH` defaults to that same path, so the env var is
//! only needed when running from another directory.
//!
//! # What this proves, and what it cannot prove here
//!
//! Provable without a reachable on-screen WindowServer (see the crate root
//! docs): that a Dart isolate runs, that
//! `setMessageHandlerOnChannel:binaryMessageHandler:` delivers guest messages
//! to Rust, that `sendOnChannel:message:` delivers host pushes to the guest,
//! and that the store is genuinely shared between the two. All of that is
//! in-process and observable from Rust.
//!
//! **Not** provable here, and not claimed: that a human sees anything. This
//! example is deliberately headless — no `tao` window, no `NSView`, no
//! attached `FlutterViewController` — because none of the claims need one and
//! a detached-view engine is where `FINDINGS.md` F1's backing-store retry
//! storm lives. Real mouse and keyboard input are equally out of scope and
//! remain unproven on this machine.
//!
...
//! this file. Every prior spike sent only `NSNumber(u64)`, `nil`, `NSString`
//! and dictionaries, so `serde_json`'s `float_roundtrip` feature — mandatory
//! in `duet-codec`, `duet-protocol` and this crate's dev-dependencies — had
//! **zero** coverage on this transport. Dart writes each double as a JSON
//! number produced by `double.toString()` (shortest round-trip decimal) and
//! this driver compares [`f64::to_bits`], never `==`: `0.1 + 0.2` and a
//! subnormal are exactly the inputs a parser without a correctly-rounding slow
//! path gets *nearly* right, and a `==` comparison that happened to pass on
//! one value would hide it on the next.
//!
...
//! [`duet_backend_macos::FlutterSurface`]'s own inbound size cap and its
...
//! channel handler is invoked inline on the platform thread and replies inline
//! too, so all this driver needs is for that thread's run loop to keep
//! turning. `NSRunLoop::runUntilDate:` does that directly, which keeps the
//! turn structure of this file explicit instead of hidden inside `tao`'s
//! `ControlFlow`.
...
///
/// Generous: the work here is an engine boot (~180 ms measured in Spike A),
/// a handshake, a dozen channel round trips — two of which carry a megabyte —
/// and the guest's own observation window. All of it lands in well under
/// 20 seconds when it lands at all.
...
/// Overridable via env var for other machines, matching `examples/lifecycle.rs`.
...
fn main() {
    ...
    println!("[flutter_state] App.framework path: {framework}");
    ...
    // Headless: `start_renderer` boots the engine with `allowHeadlessExecution`
    // and nothing here ever calls `attach_view`, so no window or NSView is
    // created. See the module docs on why that is deliberate.
    ...
    // channel handler (its `Drop`) before the engine shuts down, because
    // `shutDownEngine` does not clear the engine's handler map — see
    // `FlutterSurface`'s `Drop`.
    drop(surface);
...
/// Runs the platform thread's run loop for `secs`.
///
/// This is what gives the Flutter engine its thread back: the Dart isolate's
/// microtasks, its timers, and every inbound platform message are all serviced
/// from this run loop. Without it the guest never runs and nothing in this
/// file can happen.
///
/// No `unsafe` is needed for any of it — `objc2-foundation` exposes both
/// `NSDate::dateWithTimeIntervalSinceNow` and `NSRunLoop::runUntilDate` as
/// safe bindings — which is why this example, unlike the rest of the crate,
/// contains no `unsafe` blocks at all. It reaches the Objective-C world only
/// through [`MacBackend`] and [`FlutterSurface`], which own that unsafety.
fn pump_run_loop(secs: f64) {
    // `runUntilDate:` runs the *current* thread's run loop — the platform
    // thread, which is the one this whole example runs on.
