...
//! running real Dart invokes real `#[command]` functions on a real host, and one
//! of them writes the store both sides read.
//!
...
//! JavaScript, and a Dart guest speaks a `FlutterBinaryMessenger` channel and is
//! answered by invoking the reply block the engine hands the handler. A proof
...
//! ```
//!
//! That framework carries every Dart driver; this example selects the command
...
//!
//! Nothing visual. The engine runs **headless** — booted with
//! `allowHeadlessExecution` and never given a view — because none of these
//! claims need one. Only a debug/JIT `App.framework` has ever been built in this
//! environment, so nothing here says anything about release/AOT Dart.
...
/// Seconds of run loop per turn. This is what gives the Dart isolate its
...
fn main() {
    ...
    println!("[flutter_commands] App.framework path: {framework}");
    ...
    // `shutDownEngine` does not clear the engine's handler map.
    drop(surface);
...
/// Runs the platform thread's run loop for `secs`, which is what gives the Dart
/// isolate its thread back.
